...
## ----trace-trois-svm,teacher=correct,cache=TRUE-----------
plot(mod.svm1,data=df3,grid=250)
plot(mod.svm2,data=df3,grid=250)
plot(mod.svm3,data=df3,grid=250)
...
## ----svm-test-gamma-cost,teacher=correct,cache=TRUE-------
mod.svm2 <- svm(Y~.,data=df,kernel="radial",gamma=1,cost=0.0001)
mod.svm3 <- svm(Y~.,data=df,kernel="radial",gamma=1,cost=1)
mod.svm4 <- svm(Y~.,data=df,kernel="radial",gamma=1,cost=100000)
...
## ----foret1,teacher=correct,cache=TRUE--------------------
...
## ----foret2,teacher=correct,cache=TRUE--------------------
rf2 <- randomForest(type~.,data=spam,mtry=1)
rf1
rf2

## ----caret-foret-mtry,teacher=correct,cache=TRUE----------
library(caret)
...
## ----foret3,teacher=correct,cache=TRUE--------------------
rf3 <- randomForest(type~.,data=spam,mtry=unlist(sel.mtry$bestTune),importance=TRUE)
Imp <- randomForest::importance(rf3,type=1) %>% as.data.frame() %>% 
...
## ----comp-rf-ranger,teacher=correct,cache=TRUE------------
library(ranger)
...
## ----teacher=correct,cache=TRUE---------------------------
library(gbm)
L2boost <- gbm(Y~.,data=df1,n.trees = 500000,distribution="gaussian",bag.fraction = 1)
...
L2boost.sel <- gbm(Y~.,data=df1,n.trees = 10000,distribution="gaussian",
           bag.fraction = 1,train.fraction=0.75)
gbm.perf(L2boost.sel)
...
## ----cv-ada3,teacher=correct,cache=TRUE-------------------
...
## ----cv-logitboost,teacher=correct,cache=TRUE-------------
set.seed(4321)
logitboost <- gbm(type~.,data=spam1,distribution="bernoulli",interaction.depth=2,bag.fraction=1,cv.folds = 5,n.trees=500,shrinkage=0.4)
gbm.perf(logitboost)
...
## ----ent-percep-simple,teacher=correct,cache=TRUE---------
...
## ----ent-percep-simple-soft,teacher=correct,cache=TRUE----
...
## ----ent-deuxcouches1,teacher=correct,cache=TRUE----------
mod2c %>% fit(
  x=appX,
  y=appY,
  epochs=100,
...
## ----ent-deuxcouches2,teacher=correct,cache=TRUE----------
mod2cd %>% fit(
  x=appX,
  y=appY,
  epochs=150,
